...
/// Trait for generating parsers of themselves, using expression parser.
///
/// In `chumsky`, recursive expression parsing requires passing the
/// expression parser as an argument.
pub trait ChumskyRecursiveParse: Sized {
...
/// Struct with a phantom pointer to avoid introducing a lifetime in `RichError`.
#[derive(Default)]
pub struct S<'a> {
    _lifetime: PhantomData<&'a ()>,
}

type ParseError<'src> = extra::Full<RichError, (), S<'src>>;
    ...
    fn parse_from_str(s: &str) -> Result<Self, RichError> {
        ...
        let tokens = if let Some(tok) = tokens {
            ...
            return Err({
                let err = lex_errs
                    ...
                    .unwrap();
                RichError::new(Error::CannotParse(err.0), (*err.1).into())
            });
        };
        ...
        if parse_errs.is_empty() {
            Ok(ast.ok_or(RichError::new(
                Error::CannotParse("Empty AST without an error.".to_string()),
                Span::new(0, 0),
            ))?)
        } else {
...
impl ChumskyRecursiveParse for Statement {
    fn parser<'tokens, 'src: 'tokens, I, E>(
        expr: E,
    ) -> impl Parser<'tokens, I, Self, ParseError<'src>> + Clone
    where
...
impl ChumskyRecursiveParse for Assignment {
    fn parser<'tokens, 'src: 'tokens, I, E>(
        expr: E,
    ) -> impl Parser<'tokens, I, Self, ParseError<'src>> + Clone
    where
...
impl ChumskyRecursiveParse for Call {
    fn parser<'tokens, 'src: 'tokens, I, E>(
        expr: E,
    ) -> impl Parser<'tokens, I, Self, ParseError<'src>> + Clone
    where
...
impl ChumskyRecursiveParse for SingleExpression {
    fn parser<'tokens, 'src: 'tokens, I, E>(
        expr: E,
    ) -> impl Parser<'tokens, I, Self, ParseError<'src>> + Clone
    where
...
impl ChumskyRecursiveParse for MatchArm {
    fn parser<'tokens, 'src: 'tokens, I, E>(
        expr: E,
    ) -> impl Parser<'tokens, I, Self, ParseError<'src>> + Clone
    where
...
impl ChumskyRecursiveParse for Match {
    fn parser<'tokens, 'src: 'tokens, I, E>(
        expr: E,
    ) -> impl Parser<'tokens, I, Self, ParseError<'src>> + Clone
    where
